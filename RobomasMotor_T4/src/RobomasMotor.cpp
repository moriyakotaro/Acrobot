...
void RobomasMotor::membaInit(){
	...
	for(int i=0;i<9;i++){
		...
		this->is_useLQRControlM2006[i] = false;
	}
	for(int i=0; i<8; i++){
...
void RobomasMotor::setSenserDataM2006(uint8_t id, SenserData *senser){
	if( id <= 0 || id > 8)return;
    // this->Idpath[id] = type;
	LQRDataM2006[id].Senser = senser;
}
...
void RobomasMotor::setLQRgain(uint8_t type, uint8_t id, LQRGain *gain){
        switch (type){
                case M2006:
                        if( id <= 0 || id > 8)return;
                        this->Idpath[id] = type;
                        this->is_useLQRControlM2006[id] = true;
                        LQRDataM2006[id].Gain = gain;
                        break;
                default:
                        break;
        }
